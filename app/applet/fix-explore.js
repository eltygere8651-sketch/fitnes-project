...
const EXPLORE_CACHE_TTL = 1000 * 60 * 60 * 4;
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const workout = parseItems(getItemsFromPayload(workoutRes)).slice(0, 15);
    const focus = [...workout].reverse(); 
    const latin = parseItems(getItemsFromPayload(latinRes)).slice(0, 15);
    const party = parseItems(getItemsFromPayload(partyRes)).slice(0, 15);
    ...
    // Add fallback if specific sections are empty
    const fallbackItem = { 
...
`;
fs.writeFileSync('api/youtube/explore.ts', code);

// Now patch server.ts to use the exact same logic.
let serverCode = fs.readFileSync('server.ts', 'utf8');

// replace the explorer API in server.ts
