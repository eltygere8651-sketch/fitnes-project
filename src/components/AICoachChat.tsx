...
export default function AICoachChat({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: "coach",
      content:
        "¡Hola! Soy tu Coach de Gimnasio con IA. Estoy aquí para guiarte en cada paso de tu entrenamiento. Como eres amateur, mi prioridad número uno es tu técnica perfecta, seguridad y motivación constante.\n\nPregúntame sobre cualquier ejercicio de tu rutina, si sientes alguna molestia, o de qué forma respirar para dar tu 100%. ¿En qué te asesoro hoy?",
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      const coachMsg: ChatMessage = {
        ...
        role: "coach",
        content:
          "🚨 ¡Disculpa! No pude contactar con el servidor. Pero te aconsejo: baja la velocidad de ejecución, reduce el peso y bloquea tu abdomen fuerte. ¡La calidad de movimiento es lo más importante!",
        timestamp: new Date().toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
  ...
  return (
    <div
      ...
      className="bg-[#111] border border-white/5 rounded-3xl p-5 shadow-sm h-[485px] flex flex-col justify-between overflow-hidden"
    >
      {/* Header */}
      <div className="flex justify-between items-center border-b border-white/5 pb-3">
        ...
        <span className="text-[10px] bg-white/5 text-slate-400 border border-white/5 px-2.5 py-1 rounded-lg font-bold uppercase tracking-tight">
          Asesoría Técnica
        </span>
      </div>
      ...
      {messages.length < 3 && (
        <div className="bg-[#111] p-3.5 border border-white/5 rounded-2xl my-2">
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">
            Consejos rápidos que puedes preguntar:
          </p>
          <div className="flex flex-wrap gap-1.5">
            {QUICK_QUESTIONS.map((q, idx) => (
              <button
                ...
                className="text-[11px] bg-white/5 hover:bg-white/10 hover:text-emerald-500 border border-white/5 text-slate-400 py-1.5 px-3 rounded-xl transition duration-155 text-left shadow-xs cursor-pointer"
              >
                {q}
              </button>
            ))}
      ...
      <div className="flex-1 overflow-y-auto space-y-3.5 my-3 pr-1 py-1">
              ...
              <div
                className={`rounded-2xl p-3.5 text-xs leading-relaxed whitespace-pre-wrap ${
                  isUser
                    ...
                    : "bg-[#111] text-white border border-white/5 rounded-tl-none shadow-xs"
                }`}
              >
                {m.content}
                <span
                  className={`text-[9px] font-mono block text-right mt-1.5 leading-none ${
        ...
        {isLoading && (
          <div className="flex items-start gap-2.5 mr-auto max-w-[80%]">
            ...
            <div className="bg-[#111] p-3 rounded-2xl rounded-tl-none border border-white/5 flex items-center gap-2">
              <span
      ...
      >
        <input
          ...
          className="flex-1 bg-[#080808] text-white border border-white/5 focus:border-emerald-500 hover:border-white/10 text-xs px-3.5 py-3 rounded-xl outline-none shadow-sm"
        />
        <button
          type="submit"
          disabled={!inputText.trim() || isLoading}
          className="bg-emerald-500 hover:bg-emerald-400 disabled:bg-white/5 disabled:text-slate-500 text-[#111] font-bold px-4 py-3 rounded-xl transition flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.2)] cursor-pointer"
