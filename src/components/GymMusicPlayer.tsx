...
export default function GymMusicPlayer() {
  ...
  const [overrideCurrentTrack, setOverrideCurrentTrack] =
    useState<MusicTrack | null>(null);
    ...
    >
              ...
              if (expectedPlayingRef.current && youtubePlayerRef.current) {
                setTimeout(() => {
                  try {
                    ...
                    if (
                      intPlayer &&
                      typeof intPlayer.playVideo === "function"
                    ) {
                      intPlayer.playVideo();
                    }
                  } catch (err) {}
      ...
      <div className="flex-1 flex flex-row min-h-0 relative overflow-hidden">
        ...
        >
          {/* PLAYER BAR */}
          <div
            className={`${(!selectedPlaylist && !isPlaying && !overrideCurrentTrack) || trackListTab === "entertainment" ? "hidden" : !isTrackListExpanded ? "flex-1 p-3 pb-1 md:p-5 md:pb-3 flex flex-col justify-start items-center overflow-y-auto overflow-x-hidden" : "hidden md:flex flex-none p-3 border-b border-white/5"} bg-[#0a0a0b]/85  border-b border-white/10 relative shrink-0 transition-all duration-500 ease-in-out z-30`}
          >
          ...
          trackListTab === "entertainment" ? (
            <div
              className={`flex flex-col min-h-0 bg-black/40 flex-1 border-t border-white/5 shadow-[0_-10px_30px_rgba(0,0,0,0.4)] relative z-20 overflow-hidden transform-gpu ${!isTrackListExpanded && (selectedPlaylist || isPlaying || overrideCurrentTrack) ? "hidden" : "flex"}`}
            >
              {trackListTab !== "entertainment" && (
              ...
              <div className="flex flex-col flex-1 min-h-0 bg-[#030303] overflow-hidden">
                <div
                  className={`flex-1 ${trackListTab === "entertainment" ? "overflow-hidden" : "overflow-y-auto pb-[120px] sm:pb-0"} p-0 sm:p-0 premium-scrollbar relative flex flex-col`}
                >
                  {trackListTab === "entertainment" ? (
                    <React.Suspense
                      fallback={
