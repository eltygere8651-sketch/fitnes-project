...
const STANDARD_MOTIVATIONS: DailyMotivation[] = [
  {
    id: "mot1",
    text: "El verdadero fracaso es no intentarlo. Si hoy solo puedes hacer el 50%, ve y haz el 50% pero no te detengas.",
    author: "Arnold Schwarzenegger",
    category: "Disciplina",
  },
  {
    id: "mot2",
    text: "La constancia siempre supera al talento natural. Como amateur, cada repetición técnica que haces es una base indestructible para el futuro.",
    author: "Entrenador IA",
    category: "Enfoque",
  },
  {
    id: "mot3",
    text: "No tienes que ser excelente para empezar, pero tienes que empezar para poder llegar a ser excelente.",
    author: "Zig Ziglar",
    category: "Energía",
  },
  {
    id: "mot4",
    text: "El dolor de la disciplina es temporal, el dolor del arrepentimiento dura para siempre. Respira hondo y haz tu mejor esfuerzo hoy.",
    author: "Leyenda del Deporte",
    category: "Disciplina",
  },
  {
    id: "mot5",
    text: "Tus músculos crecen durante el descanso y la nutrición correcta, no solo levantando. Trata tu fase de recuperación con el mismo respeto que tu entrenamiento.",
    author: "Fisioterapeuta del Equipo",
...
export default function MotivationalNotifications({
  ...
  const [quoteOfTheDay, setQuoteOfTheDay] = useState<DailyMotivation>(
    STANDARD_MOTIVATIONS[0],
  );
  const [customGoal, setCustomGoal] = useState("");
  const [soundEnabled, setSoundEnabled] = useState(true);
  const [notificationsQueue, setNotificationsQueue] = useState<
    { id: string; msg: string; timestamp: string }[]
  >([
    {
      id: "init1",
      msg: "🎯 ¡Lista de reproducción cargada! Recuerda activar el reproductor Phonk en la pestaña de música.",
      timestamp: "Hace un momento",
    },
    {
      id: "init2",
      msg:
        "🔥 ¡Tu racha actual de entrenamiento es de " +
        activeStreak +
        " días deportivos! Sigue así.",
...
Usa máximo 2 líneas. Ofrece un tono empático, firme y profesional de Coach Deportivo de Elite, que impulse mi disciplina hoy.`,
      ...
      if (data && data.reply) {
        ...
        pushNotification(
          "🌟 Nueva motivación generada: " +
            data.reply.substring(0, 45) +
            "...",
        );
      }
    } catch (err) {
  ...
  return (
    <div
      id="motivational-notifications-container"
      className="grid grid-cols-1 md:grid-cols-12 gap-6"
    >
      {/* Configuration & Preferences Side */}
      <div className="col-span-1 md:col-span-5 bg-[#111] border border-white/5 rounded-3xl p-5 shadow-sm flex flex-col justify-between gap-5 animate-fadeIn">
        <div>
          <span className="bg-emerald-500/10 text-indigo-650 text-xs font-semibold px-2.5 py-1 rounded-full border border-indigo-100 flex items-center gap-1.5 w-fit">
            <Bell className="w-3.5 h-3.5 text-emerald-500 animate-pulse" />{" "}
            Sistema Sensorial y Alertas
          </span>
          <h2 className="text-xl font-sans font-bold text-white mt-2">
            Recordatorios Diarios
          </h2>
          <p className="text-xs text-slate-400 mt-1">
            Configura alertas de hidratación, respiración, postura e incentivos
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 bg-white/5 border border-slate-150 rounded-xl shadow-xs">
            <div>
              <p className="text-xs font-bold text-white">
                Notificaciones Interactivas
              </p>
              <p className="text-[10px] text-slate-400 font-medium">
                Incentivos motivacionales y sonidos en intervalos
              </p>
            </div>
          ...
          <div className="flex items-center justify-between p-3 bg-white/5 border border-slate-150 rounded-xl shadow-xs">
            <div>
              <p className="text-xs font-bold text-slate-800">
                Efectos de Audio de Logros
              </p>
              <p className="text-[10px] text-slate-400 font-medium">
                Genera tonos sintéticos al entrenar
              </p>
            </div>
            <button
              ...
              className={`p-1.5 rounded-lg border cursor-pointer transition ${
                soundEnabled
                  ? "bg-indigo-55 bg-emerald-500/10 text-emerald-500 border-indigo-150"
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-slate-405 uppercase tracking-wide">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#080808] text-white border border-white/5 focus:border-emerald-500 text-xs px-3.5 py-2.5 rounded-xl outline-none shadow-xs"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            onClick={() =>
              pushNotification(
                "💧 Recordatorio: ¡Hora de tomar 250ml de agua de inmediato!",
              )
            }
            className="flex-1 bg-white hover:bg-slate-50 border border-white/5 text-[10px] text-slate-400 font-bold py-2 rounded-xl transition cursor-pointer shadow-xs"
          >
          ...
          <button
            onClick={() =>
              pushNotification(
                "🧘 Postura: Reajusta tus hombros hacia atrás y relaja el cuello hoy.",
              )
            }
      ...
      <div className="col-span-1 md:col-span-7 space-y-4 animate-fadeIn">
        ...
        <div className="bg-[#111] border border-white/5 rounded-3xl p-6 relative overflow-hidden shadow-sm">
          ...
          <div className="flex items-center justify-between mb-4 relative z-1">
            <div className="flex items-center gap-2">
              <span
                className={`text-[10px] font-mono tracking-wider font-bold px-2.5 py-1 rounded uppercase ${
          ...
          <div className="space-y-4 relative z-1">
            ...
            <div className="flex items-center justify-between pt-1 flex-wrap gap-2">
              <p className="text-xs text-slate-400 block font-bold font-sans">
              ...
              <button
                ...
                disabled={loadingAI}
                className="bg-emerald-500 hover:bg-indigo-700 disabled:bg-slate-100 disabled:text-slate-400 text-white font-bold text-xs px-3.5 py-2 rounded-xl transition duration-200 flex items-center gap-1 shadow-sm cursor-pointer"
              >
                {loadingAI ? (
                  <>Generando...</>
                ) : (
                  <>
                    <Sparkles className="w-3.5 h-3.5" /> Motivación IA
                    Personalizada
                  </>
                )}
              </button>
        ...
        <div className="bg-slate-50/50 border border-slate-200 rounded-2xl p-4 space-y-3 shadow-xs">
          <div className="flex justify-between items-center border-b border-slate-150 pb-2">
            <h4 className="text-xs font-bold text-slate-600 flex items-center gap-1.5 uppercase tracking-wide">
              <CheckCircle2 className="w-3.5 h-3.5 text-indigo-600" /> Registro
              Reciente de Consejos de Hoy
            </h4>
            <button
          ...
          <div className="space-y-2 max-h-[140px] overflow-y-auto pr-1">
            {notificationsQueue.length === 0 ? (
              <p className="text-[11px] text-slate-450 text-center py-4">
                No hay alertas. Se activarán en base a tus entrenamientos.
              </p>
            ) : (
              notificationsQueue.map((item) => (
                <div
                  key={item.id}
                  className="flex items-start gap-2.5 bg-white hover:bg-slate-50/60 transition p-2.5 rounded-xl border border-slate-150 shadow-xs"
                >
