...
const EXERCISE_GUIDES: ExerciseGuideDetails[] = [
  ...
  },
  {
    ...
    tipsForAmateurs:
      "Como amateur, si te resulta difícil ejecutarlas en el suelo con técnica estable, por favor apoya las rodillas o apoya las manos en una superficie elevada (banco o pared) para reducir la carga de forma amigable.",
    cueAnglePoints: [
  ...
  },
  {
    ...
    tipsForAmateurs:
      "Sostener las mancuernas al inicio del movimiento frente a tus ojos te ayuda a tener un rango de movimiento mucho más natural y fluido que el clásico press de hombros.",
    cueAnglePoints: [
  ...
  },
  {
    ...
    tipsForAmateurs:
      "Para comprobar tu técnica, pon una toalla delgada bajo tu espalda baja e intenta que alguien tire de ella. Si logran sacarla, significa que debes empujar tu ombligo más hacia adentro.",
    cueAnglePoints: [
...
export default function TechGuide({
  ...
  onAskCoach: (exerciseName: string) => void;
}) {
  const [selectedGuide, setSelectedGuide] = useState<ExerciseGuideDetails>(
    EXERCISE_GUIDES[0],
  );
  const [postureTab, setPostureTab] = useState<"correct" | "incorrect">(
    "correct",
  );
    ...
    >
      ...
      <div className="col-span-1 lg:col-span-4 space-y-3">
        <h3 className="text-xs font-bold font-sans text-slate-400 uppercase tracking-wider flex items-center gap-1.5 px-1">
          <Layers className="w-4 h-4 text-emerald-500" /> Videoguías Correctivas
        </h3>
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3.5 rounded-2xl border transition duration-150 flex items-center justify-between gap-3 ${
                  isSelected
                    ? "bg-[#111] border-indigo-500 shadow-md shadow-indigo-100 text-white"
                    : "bg-white border-white/5 hover:bg-white/5 hover:border-slate-300 text-slate-400 shadow-sm"
                }`}
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-mono uppercase bg-white/5 text-slate-400 px-1.5 rounded border border-white/5">
                      {ex.category}
                    </span>
                    <span
                      className={`text-[9px] font-semibold font-mono px-1.5 rounded ${
                  ...
                  <p className="font-bold text-sm truncate mt-1.5 text-white">
                    {ex.name}
                  </p>
                </div>

                <span className="text-xs bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-lg w-7 h-7 flex items-center justify-center shrink-0 font-bold">
                  ⚡
                </span>
              </button>
            );
        ...
        <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-start gap-3 shadow-sm">
          ...
          <p className="text-[11px] text-slate-400 leading-normal">
            <strong className="text-emerald-500 block font-bold uppercase tracking-wider">
              Consejo para Novatos:
            </strong>
            Filtrar los errores comunes te ayuda a prevenir el 90% de lesiones y
            sobrecargas articulares típicas en amateurs.
          </p>
        </div>
      </div>
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Guía de Postura Bioquímica • {selectedGuide.category}
              </p>
            </div>

            <div className="flex items-center gap-1.5 self-end sm:self-auto bg-[#050505] border border-white/5 p-1 rounded-xl shadow-inner">
              <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            ...
            <div className="bg-white/5 border border-white/5 rounded-2xl p-4 space-y-2.5">
              <h4 className="text-xs font-bold text-emerald-600 flex items-center gap-1.5 uppercase tracking-wide">
                <CheckCircle2 className="w-4 h-4" /> Lo que DEBES HACER
                (Postura)
              </h4>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
            ...
            <div className="space-y-2">
              {selectedGuide.stepByStep.map((step, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-3 text-[11px] text-slate-600"
                >
                  <span className="w-5 h-5 bg-[#111] border border-white/5 text-[10px] font-mono font-bold text-emerald-500 rounded-lg flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
                    {idx + 1}
                  </span>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            <p className="text-[11px] text-slate-600 text-center sm:text-left">
              ¿Tienes molestias musculares o dudas sobre cuánto peso iniciar en
              el{" "}
              <strong className="text-white">{selectedGuide.name}</strong>?
            </p>

            <button
              onClick={() => onAskCoach(selectedGuide.name)}
              className="bg-emerald-500 hover:bg-emerald-400 text-black shadow-[0_0_15px_rgba(16,185,129,0.3)] font-bold text-xs px-4 py-2 rounded-xl transition duration-200 flex items-center gap-1.5 shrink-0 shadow-md shadow-indigo-100 cursor-pointer"
            >
              <MessageCircle className="w-4 h-4" /> Preguntar al Coach Técnico
              de IA
            </button>
          </div>
        </div>
